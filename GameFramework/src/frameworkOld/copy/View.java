...
public abstract class View extends JFrame {
	...
	public void addListener(Controller controller) throws GameException {
		if (!(controller instanceof KeyListener) || !(controller instanceof WindowListener)
	...
	protected void render(Graphics2D gc, List<Model> list) throws GameException {
		...
		for (int index = 0; index < list.size(); index++) {
			...
			Rectangle viewPortRectangle = new Rectangle(ViewPaddingLeft, ViewPaddingTop,
					ViewWidth - ViewPaddingRight - dstDim.width, ViewHeight - ViewPaddingBottom - dstDim.height);
	...
	public void displayMessage(String message, Graphics2D gc, Color color, int offset) {
	...
	public void displayMessage(String message, Graphics2D gc, Color color, int x, int y) {
		Color oldColor = gc.getColor();
		Font oldFont = gc.getFont();
		Font font = new Font("Courier New", Font.PLAIN, 16);
		gc.setFont(font);
