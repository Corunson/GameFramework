...
public class Sprite implements Iterable<Image> {
	...
	public Sprite(String resid, Component parent) {
		this.parent = parent;
		this.resid = resid;
		...
		for (int i = 0; i < imageBounds.length; i++) {
			imageBounds[i] = new Rectangle(i * imageSrcDimension.width, 0, 
											imageSrcDimension.width, imageSrcDimension.height);
		}
	}
	...
	private Image retrieveImage(String resid) {
		...
		try {
			...
			System.err.println("Can't load image " + resid);
			System.exit(0);	
		}
		
		GraphicsConfiguration graphicsConfig = GraphicsEnvironment.getLocalGraphicsEnvironment().
				getDefaultScreenDevice().getDefaultConfiguration();
		Image image = graphicsConfig.createCompatibleImage(srcImage.getWidth(), 
